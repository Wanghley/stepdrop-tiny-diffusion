...
\begin{table}[h]
  ...
  \begin{tabular}{lcccc}
    ...
    StepDrop (Target-25, importance) & 25 & 77.61 & 4.13 & \textbf{96.8} \\
    \bottomrule
  \end{tabular}
\end{table}

All 50-step methods cost $\approx$59 GFLOPs (total per image); 25-step methods cost $\approx$29.5 GFLOPs (total per image).
The throughput gap between StepDrop and DDIM arises from Python-side scheduling overhead in our prototype implementation, not from fundamental algorithmic cost.
